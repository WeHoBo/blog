<template>
  <div>
    ...
    <div v-else class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 sm:p-6 space-y-4">
      ...
      <input v-model="form.summary" placeholder="摘要" aria-label="文章摘要" class="w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 text-sm" />
      <div>
        <label class="text-sm text-gray-500 mb-1 block" for="edit-cover">封面图</label>
        <div class="flex gap-2">
          <input id="edit-cover" v-model="form.cover" placeholder="封面图URL" class="flex-1 px-3 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 text-sm" />
